...
# Helper function for safe RSF prediction with NA handling
# 用于安全RSF预测的辅助函数，处理缺失值
safe_predict_vector <- function(fit, newdata, predict_args = list()) {
  n <- nrow(newdata)
  RS <- rep(NA_real_, n)
  if (ncol(newdata) > 2) {
    predictor_rows <- complete.cases(newdata[, -c(1,2), drop = FALSE])
    idx <- which(predictor_rows)
  } else {
    idx <- seq_len(n)
  }
  if (length(idx) > 0) {
    args <- c(list(object = fit, newdata = newdata[idx, , drop = FALSE]), predict_args)
    pr <- tryCatch(do.call(predict, args), error = function(e) e)
    if (inherits(pr, "error")) {
      warning("Prediction failed: ", conditionMessage(pr))
    } else {
      if (is.list(pr) && !is.null(pr$predicted)) pr_vec <- as.numeric(pr$predicted) else pr_vec <- as.numeric(pr)
      if (length(pr_vec) != length(idx)) {
        warning("Prediction length (", length(pr_vec), ") does not match expected (", length(idx), "). Filling available values; others remain NA.")
        m <- min(length(pr_vec), length(idx))
        RS[idx[seq_len(m)]] <- pr_vec[seq_len(m)]
      } else {
        RS[idx] <- pr_vec
      }
    }
  }
  return(cbind(newdata[, 1:2, drop = FALSE], RS = RS))
}
...
fit <- rfsrc(Surv(OS.time,OS)~., data = est_dd, 
             ...
             forest = T, 
             seed = seed)
rs <- lapply(val_dd_list, function(x) safe_predict_vector(fit, x, predict_args = list()))
