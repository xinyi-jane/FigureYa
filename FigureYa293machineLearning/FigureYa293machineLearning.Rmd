...
# Helper function to make predictions robust to missing predictor rows
# 辅助函数使预测对缺失预测值行具有鲁棒性
safe_predict_vector <- function(fit, newdata, predict_args = list()) {
  # newdata expected to include survival cols in columns 1:2
  n <- nrow(newdata)
  RS <- rep(NA_real_, n)
  # identify rows with complete predictor values (exclude first two columns)
  if (ncol(newdata) > 2) {
    idx <- which(complete.cases(newdata[, -c(1,2)]))
  } else {
    idx <- seq_len(n)
  }
  if (length(idx) > 0) {
    ...
    if (inherits(pr, "error")) {
      ...
      if (is.list(pr) && !is.null(pr$predicted)) pr_vec <- pr$predicted else pr_vec <- as.numeric(pr)
      RS[idx] <- pr_vec
    }
  }
